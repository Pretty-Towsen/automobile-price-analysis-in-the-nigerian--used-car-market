...
#use config page
st.set_page_config(
    page_title="Automation Analysis Dashboard",
    page_icon="🚕",
    layout="wide"
)
...
def load_data():
    try:
        df = pd.read_csv("data/automation_price_analysis_cleaned_data.csv")
        return df
...
def display_chart(filtered_df):
    ...
    with col2:
        ...
        avg_price = (filtered_df.groupby('Make')['Price'].mean().sort_values(ascending=False).reset_index()
        )
        fig2 = px.bar(
        avg_price,
        x='Make',
